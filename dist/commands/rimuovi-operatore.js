...
exports.data = new discord_js_1.SlashCommandBuilder()
    ...
    .setDescription('Rimuove un utente dagli Operatori Ufficiali C.S.D.')
    .addUserOption((option) => option
    .setName('utente')
    .setDescription('Utente da rimuovere dagli Operatori Ufficiali C.S.D.')
    .setRequired(true));
async function execute(interaction) {
    ...
    const removed = (0, staffManager_1.removeOperatore)(user.id);
    if (!removed) {
        await interaction.reply({
            content: `⚠️ ${user} non è un Operatore Ufficiale C.S.D.`,
            flags: discord_js_1.MessageFlags.Ephemeral,
        });
        return;
    }
    await interaction.reply({
        content: `✅ ${user} è stato rimosso dagli **Operatori Ufficiali C.S.D.**`,
        flags: discord_js_1.MessageFlags.Ephemeral,
    });
}
