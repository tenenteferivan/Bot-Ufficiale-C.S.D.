...
exports.data = new discord_js_1.SlashCommandBuilder()
    .setName('annuncio-sicurezza')
    .setDescription('Invia un avviso di sicurezza ai canali log configurati.')
    .addStringOption((option) => option
    .setName('messaggio')
    .setDescription('Contenuto dell\'avviso di sicurezza')
    .setMaxLength(4096)
    .setRequired(true));
async function execute(interaction) {
    ...
    try {
        const result = await (0, announcementDistributor_1.distributeAnnouncement)(interaction.client, (0, announcementDistributor_1.buildAnnouncementEmbed)('🚨 Avviso di sicurezza', message, 0xed4245), '@everyone');
        await interaction.editReply({
            content: `✅ Avviso di sicurezza distribuito in ${result.sent} server.` +
                (result.failed ? `\n⚠️ ${result.failed} destinazioni non hanno potuto riceverlo.` : ''),
        });
    }
    catch (error) {
        console.error('Errore durante la distribuzione dell\'avviso di sicurezza:', error);
        await interaction.editReply({
            content: '❌ Non è stato possibile distribuire l\'avviso di sicurezza.',
        });
    }
}
