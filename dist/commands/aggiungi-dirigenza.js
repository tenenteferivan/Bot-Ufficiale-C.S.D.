...
exports.data = new discord_js_1.SlashCommandBuilder()
    ...
    .setDescription('Nomina un utente alla Dirigenza Esecutiva di C.S.D.')
    .addUserOption((option) => option
    .setName('utente')
    .setDescription('Utente da nominare alla Dirigenza Esecutiva di C.S.D.')
    .setRequired(true));
async function execute(interaction) {
    ...
    const added = (0, staffManager_1.addDirigenza)(user.id);
    if (!added) {
        await interaction.reply({
            content: `⚠️ ${user} è già membro della **Dirigenza Esecutiva di C.S.D.**`,
            flags: discord_js_1.MessageFlags.Ephemeral,
        });
        return;
    }
    await interaction.reply({
        content: `✅ ${user} è stato nominato alla **Dirigenza Esecutiva di C.S.D.**`,
        flags: discord_js_1.MessageFlags.Ephemeral,
    });
}
