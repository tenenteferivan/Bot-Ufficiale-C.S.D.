...
exports.sendTicketTranscript = sendTicketTranscript;
...
function messageHtml(message) {
    ...
    const attachments = [...message.attachments.values()].map((attachment) => (`<li><a href="${escapeHtml(attachment.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(attachment.name ?? 'Allegato')}</a></li>`)).join('');
...
function buildHtml(channel, ticket, closer, reason, messages) {
    const category = ticketManager_1.ticketCategories[ticket.category];
