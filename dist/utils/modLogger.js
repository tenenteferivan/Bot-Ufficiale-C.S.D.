...
async function sendModNotification(interaction) {
    ...
    if (['aggiungi', 'rimuovi', 'registro', 'pulisci'].includes(interaction.commandName)
        || (interaction.commandName === 'warn' && subcommand === 'remove'))
        return false;
    ...
    if (!targetUserId || targetUser?.bot)
        return false;
    ...
    const embed = new discord_js_1.EmbedBuilder()
        ...
        .setColor(color)
        .addFields({ name: '📋 Motivo', value: reason, inline: false }, { name: '🛡️ Moderatore', value: `${interaction.user.tag}`, inline: true })
        .setTimestamp();
