import { APIInteractionGuildMember, EmbedBuilder, GuildMember, PermissionFlagsBits, User } from 'discord.js';
...
export function parseWarningDuration(input: string): number | null {
  ...
  if (isNaN(days) || days < 1 || days > 30) return null;
  return days;
}

export function getUserSanctions(guildId: string, userId: string): Promise<UserSanctionEntry[]> {
  return new Promise((resolve, reject) => {
    db.all<any>(
      'SELECT * FROM user_sanctions WHERE guild_id = ? AND user_id = ? ORDER BY id ASC',
      [guildId, userId],
      (err, rows) => {
...
export async function getUserRecord(guildId: string, userId: string): Promise<UserRecord> {
...
export function updateUserPoints(
  ...
  return new Promise((resolve, reject) => {
    db.run(
      `INSERT INTO user_records (guild_id, user_id, points, max_points)
       VALUES (?, ?, ?, 20.0)
       ON CONFLICT(guild_id, user_id) DO UPDATE SET points = excluded.points`,
      [guildId, userId, newPoints],
...
export async function removePoints(
  ...
  await new Promise<void>((resolve, reject) => {
    db.run(
      `INSERT INTO user_sanctions (guild_id, user_id, type, reason, points_removed, created_at)
       VALUES (?, ?, 'POINTS_REMOVAL', ?, ?, ?)`,
      [guildId, userId, reason, amount, createdAt],
      (err) => (err ? reject(err) : resolve())
    );
  });
...
export async function addNote(
  ...
  return new Promise<void>((resolve, reject) => {
    db.run(
      `INSERT INTO user_notes (guild_id, user_id, note, created_at)
       VALUES (?, ?, ?, ?)`,
      [guildId, userId, note, new Date().toISOString()],
      (err) => (err ? reject(err) : resolve())
    );
  });
} 
...
export async function addFormalWarning(
  ...
  await new Promise<void>((resolve, reject) => {
    db.run(
      `INSERT INTO user_sanctions (guild_id, user_id, type, reason, duration_days, created_at, expires_at)
       VALUES (?, ?, 'FORMAL_WARNING', ?, ?, ?, ?)`,
      [guildId, userId, reason, durationDays, now.toISOString(), expiresAt.toISOString()],
      (err) => (err ? reject(err) : resolve())
    );
  });
...
export async function createUserInfoEmbed(
  ...
  } else if (member && 'joined_at' in member && (member as any).joined_at) {
    const parsed = new Date((member as any).joined_at).getTime();
    if (!isNaN(parsed)) {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      { name: '👤 Nome Discord', value: user.globalName ? `${user.globalName} (\`${user.username}\`)` : user.username, inline: true },
      { name: '🏷️ Handle (@)', value: `@${user.username}`, inline: true },
      { name: '🆔 ID', value: `\`${user.id}\``, inline: true },
      { name: '📅 Creazione Account', value: `<t:${createdTimestamp}:F> (<t:${createdTimestamp}:R>)`, inline: true },
      { name: '📥 Entrato nel Server', value: joinedTimestamp ? `<t:${joinedTimestamp}:F> (<t:${joinedTimestamp}:R>)` : 'Non disponibile', inline: true },
      { name: '──────────────', value: '📁 **Registro Utente**', inline: false },
      { name: '📜 Storico Sanzioni', value: sanctionsHistory.slice(0, 1024), inline: false },
      { name: '🪙 Totale punti', value: `\`${formattedPoints}\``, inline: true },
      { name: '📝 Note', value: notes, inline: true },
      { name: '🚨 Segnalazioni a Carico', value: reports, inline: true },
