...
import { getSanctions, saveSanction } from '../../handlers/databasehandler';
...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  if (subcommand === 'add') {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { name: '👤 Utente Avvertito', value: `<@${targetUser.id}>\n\`ID: ${targetUser.id}\``, inline: true },
    ...
    const warn = (await getSanctions(targetUser.id, interaction.guild.id)).find(
      (record) => record.id === Number(warnId) && record.type === 'WARN'
    );
    ...
    if (warn) {
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          { name: '👤 Utente', value: `<@${targetUser.id}>`, inline: true },
          { name: '🆔 ID Avvertimento Rimosso', value: `\`${warnId}\``, inline: true },
          { name: '🛡️ Moderatore', value: `<@${interaction.user.id}>`, inline: true }
        )
        .setTimestamp();
      ...
      await targetUser.send({
        embeds: [
          new EmbedBuilder()
            ...
            .addFields(
              { name: '📋 Motivo', value: `Revoca dell'avvertimento: ${warn.reason}`, inline: false },
...
async function getWarnsFromDatabase(userId: string, guildId?: string): Promise<{ id: number; reason: string }[]> {
  if (!guildId) return [];
  const sanctions = await getSanctions(userId, guildId);
  return sanctions
