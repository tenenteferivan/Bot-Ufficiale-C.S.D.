    ...
    if (interaction.isModalSubmit()) {
      ...
      if (
        ...
        try {
          ...
          console.log(
            '[SEGNALAZIONE] Modal ricevuto correttamente.'
          );
          ...
          const reportId = await addUserReport(
            userId,
            interaction.user.id,
            motivation,
            facts,
          ...
          console.log(
            `[SEGNALAZIONE] Segnalazione salvata con ID #${reportId}.`
          );
          ...
          for (const config of configs) {
            ...
            try {
              ...
              if (!guild) {
                console.error(
                  `[SEGNALAZIONE] Server non trovato: ${config.guildId}`
                );
              ...
              if (
                ...
                console.error(
                  `[SEGNALAZIONE] Canale non valido: ${config.channelId}`
                );
              ...
              console.error(
                `[SEGNALAZIONE] Errore nel server ${config.guildId}:`,
                error
              );
            }
          }
          ...
          await interaction.editReply({
            content:
              `✅ Segnalazione **#${reportId}** registrata correttamente.\n\n` +
          ...
          console.log(
            `[SEGNALAZIONE] Operazione completata. ` +
          ...
          if (
            ...
            await interaction.editReply({
              content:
                `❌ Si è verificato un errore: ${errMsg}`,
            }).catch(() => {});
          } else {
      ...
      if (
        ...
        try {
          ...
          console.log(
            '[SANZIONE] Modal ricevuto correttamente.'
          );
          ...
          console.log(
            '[SANZIONE] Dati della sanzione recuperati.'
          );
          ...
          const embed = new EmbedBuilder()
            .setColor(0xC0392B)
            .setTitle(
              '⚖️ Nuova Sanzione Disciplinare'
            )
            .setDescription(
              'È stata registrata una nuova sanzione disciplinare a carico di un utente.'
            )
            .addFields(
              {
            ...
            .setFooter({
              text:
                'Sistema Sanzioni Disciplinari C.S.D.',
            })
            .setTimestamp();
          ...
          console.log(
            '[SANZIONE] Recupero configurazioni canali...'
          );

       const configs =
  await getSanctionConfigs();


          console.log(
            `[SANZIONE] Canali configurati: ${configs.length}`
          );
          ...
          if (configs.length === 0) {
            ...
            await interaction.editReply({
              content:
                '❌ Non è stato configurato alcun canale per la pubblicazione delle sanzioni.',
            });
          ...
          for (const config of configs) {
            ...
            try {
              ...
              console.log(
                `[SANZIONE] Invio al server ${config.guildId}, ` +
              ...
              if (!guild) {
                ...
                console.error(
                  `[SANZIONE] Server non trovato: ${config.guildId}`
                );
              ...
              if (
                ...
                console.error(
                  `[SANZIONE] Canale non valido: ${config.channelId}`
                );
              ...
              console.log(
                `[SANZIONE] Sanzione pubblicata correttamente ` +
              ...
              console.error(
                `[SANZIONE] Errore nel server ${config.guildId}:`,
                error
              );
            }
          }
          ...
          await interaction.editReply({
            content:
              `✅ Sanzione disciplinare registrata correttamente.\n\n` +
              `📡 Pubblicata in **${sent}** server.` +
              (
                failed > 0
                  ? `\n⚠️ Non è stato possibile pubblicarla in **${failed}** server.`
          ...
          console.log(
            `[SANZIONE] Operazione completata. ` +
          ...
          if (
            ...
            await interaction.editReply({
              content:
                `❌ Si è verificato un errore durante la registrazione della sanzione: ${errMsg}`,
            }).catch(() => {});
